...
totalnoisedata = np.load('compliednoisedatasnr.pkl.npy')
...
text = 'The 1% threshold is values above:{th99}\nThe 0.1% threshold is values above:{th999}\nThere are {b} bins in this histogram of {l} noise measurements.'.format(th99=threshold99,th999=threshold999,b=binrange,l=len(xsorted))

#the histogram of the data
plt.figure()
n, bins, patches = plt.hist(xsorted, binrange, density=True, facecolor='royalblue', alpha=0.75,label='Background Noise Data')
plt.xlabel('SNR of Background Noise')
plt.ylabel('Frequency density')
plt.title('Histogram of Generated Background Signal SNR\'s')
#plt.text(4.25,-0.5,text,ha='left',wrap=True)
